// src/pages/PublicCetakPage.tsx
import { useEffect, useState, useCallback } from 'react';
import { useParams } from 'react-router-dom';
import { Title, Loader, Text, Group, Table, Container } from '@mantine/core';
...
import { fetchPublicPrintRedirect} from '../api/skpService'; // Gunakan fungsi API publik
// ... import lainnya
...
export function PublicCetakPage() {
    // Ambil parameter dari URL publik
        // --- LENGKAPI BAGIAN INI ---
        // Mengambil parameter nip, year, dan month dari URL
        const { nip, year, month } = useParams<{ nip: string, year: string, month: string }>();
        const [periodeDetail, setPeriodeDetail] = useState<PeriodePenilaianDetail | null>(null);
        const [loading, setLoading] = useState(true);
    
        // --- LENGKAPI FUNGSI FETCHDATA INI ---
        const fetchData = useCallback(async () => {
            // Pastikan semua parameter ada sebelum fetching
            if (!nip || !year || !month) {
                notifications.show({ color: 'red', message: 'URL tidak valid: NIP, tahun, atau bulan tidak ditemukan.' });
                setLoading(false);
                return;
            }
            setLoading(true);
            try {
                // Panggil fungsi API publik dengan parameter dari URL
                const data = await fetchPublicPrintRedirect(nip, Number(year), Number(month));
                setPeriodeDetail(data);
            } catch (error) {
                notifications.show({ color: 'red', message: 'Gagal memuat data penilaian.' });
            } finally {
                setLoading(false);
            }
        }, [nip, year, month]); // Dependensi: jalankan ulang jika salah satu parameter berubah
